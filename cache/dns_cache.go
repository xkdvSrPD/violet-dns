...
// MemoryDNSCache 内存 DNS 缓存（RR 级别）
...
func (c *MemoryDNSCache) GetRRs(qname string, qtype uint16) ([]*RRCacheItem, bool) {
	key := CacheKey{
		Name:  dns.Fqdn(qname),
		Type:  qtype,
		Class: dns.ClassINET,
...
// SetRRs 缓存多条 RR 记录
func (c *MemoryDNSCache) SetRRs(qname string, qtype uint16, items []*RRCacheItem) error {
	if len(items) == 0 {
		return nil
	}
...
// SetSingleRR 缓存单条 RR 记录
func (c *MemoryDNSCache) SetSingleRR(item *RRCacheItem) error {
	hdr := item.RR.Header()
	key := CacheKey{
		Name:  hdr.Name,
		Type:  hdr.Rrtype,
...
// DeleteRRs 删除指定 qname 和 qtype 的所有 RR 记录
func (c *MemoryDNSCache) DeleteRRs(qname string, qtype uint16) error {
	key := CacheKey{
		Name:  dns.Fqdn(qname),
		Type:  qtype,
		Class: dns.ClassINET,
...
func ResolveCNAMEChain(cache DNSCache, qname string, qtype uint16, maxDepth int) ([]dns.RR, bool, string) {
	if maxDepth <= 0 {
		maxDepth = 10 // 默认最大深度
	}
...
func CacheResponseByRR(cache DNSCache, msg *dns.Msg) error {
